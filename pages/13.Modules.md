...
ECMAScript 2015와 함께 하면서, JavaScript는 모듈 컨셉(concept)을 가지게 되었다. TypeScript는 이 컨셉을 고유한다.
...
모듈들은 자체 스코프에서 실행된다. 글로벌 스코프가 아니다. 이것은 모듈에서 선언된 변수, 함수, 클래스 등이 모듈 외부에서 노출되지 않는다. [`export` 형태](#export)를 사용해서 명시적으로 내보내지 않는 이상.
반대로, 다른 모듈에서 내보낸 변수, 함수, 클래스, 인터페이스 등을 사용하기 위해서는, [`import` 형태](#export)로 사용해서 임포트 해야 한다.
